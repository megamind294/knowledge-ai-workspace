...
The database password must be URL-safe because Compose interpolates it into the API's PostgreSQL URL. `docker compose down` preserves both named volumes; add `--volumes` only when deliberately deleting local database and document data. The GitHub Actions `container-smoke` job builds these images and starts this topology before exercising web health, API liveness/readiness, registration, login, authenticated identity, deep SPA routing, and non-root runtime users. Docker and Podman were unavailable in the implementation environment, so local verification was limited to mutation-tested static contracts. The complete image build and runtime smoke passed on implementation commit `cd1ba104f0f61ceb36fec255ca096c7f3a09da4b` in [GitHub Actions run #146](https://github.com/megamind294/knowledge-ai-workspace/actions/runs/34467682308).
...
6. **Day 6 — complete:** CI-verified operational readiness, structured observability, hardened production images, Compose runtime smoke, Chromium end-to-end/accessibility acceptance, and a deployment/operations runbook
