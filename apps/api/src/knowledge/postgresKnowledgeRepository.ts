...
export class PostgresKnowledgeRepository implements KnowledgeRepository {
  ...
  async authorizeDocumentUpload(userId:string, workspaceId:string, documentId:string) { await this.requireWrite(this.pool,userId,workspaceId); const value=await this.getDocument(userId,workspaceId,documentId); if(!value) throw new KnowledgeRepositoryError("NOT_FOUND","Document not found"); return value; }
