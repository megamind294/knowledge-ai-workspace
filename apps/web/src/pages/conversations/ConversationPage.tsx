    ...
    <section className="mx-auto max-w-5xl">
      ...
      <section className="mt-8 space-y-4" aria-label="Conversation history">
        ...
        {messages.map((message)=><article className={`rounded-2xl border p-5 ${message.role === "assistant" ? "border-indigo-300/20 bg-indigo-950/20" : "border-white/10 bg-slate-900/60"}`} key={message.id}>
          ...
          {message.role === "assistant" && message.sources.length > 0 ? <section aria-label="Sources for answer" className="mt-5"><h3 className="text-sm font-semibold text-indigo-200">Sources</h3><ol className="mt-3 grid gap-3">{message.sources.map((source)=><li className="rounded-xl border border-white/10 bg-slate-950/60 p-4" key={source.chunkId}><div className="flex flex-wrap justify-between gap-3"><p className="font-medium">[{source.citationOrdinal + 1}] {source.originalFilename}</p><Link aria-label={`Open ${source.originalFilename}`} className="text-sm text-indigo-300" to={`/app/documents/${source.documentId}`}>Open source</Link></div><p className="mt-2 text-xs text-slate-400">{source.sectionHeading ?? `Passage ${source.ordinal + 1}`}{source.pageNumber ? ` · Page ${source.pageNumber}` : ""}</p><p className="mt-3 text-sm leading-6 text-slate-300">{source.content}</p></li>)}</ol></section> : null}
        </article>)}
