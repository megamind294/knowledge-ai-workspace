...
export default class PrivacySafeReporter {
  ...
  onTestEnd(_test, result) {
    ...
    this.results.push({ status, duration: Number.isFinite(result.duration) ? result.duration : 0 });
  ...
  async onEnd(result) {
    const failures = this.results.filter(({ status }) => status !== "passed" && status !== "skipped").length;
    const skipped = this.results.filter(({ status }) => status === "skipped").length;
    const cases = this.results.map(({ status, duration }, index) => {
    ...
    const document = `<?xml version="1.0" encoding="UTF-8"?><testsuite name="browser-smoke" tests="${this.results.length}" failures="${failures}" skipped="${skipped}">${cases}</testsuite>`;
    await mkdir(dirname(this.outputFile), { recursive: true });
    await writeFile(this.outputFile, document, "utf8");
    this.writeLine(JSON.stringify({ event: "browser_tests_finished", status: result.status, total: this.results.length, failures, skipped }));
  }
}
